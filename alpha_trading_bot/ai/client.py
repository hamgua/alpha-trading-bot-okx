...
【⚡ 关键分析要求】
1. 趋势敏感性：当趋势强度>0.3时，必须考虑趋势影响，不得忽视明显的价格方向
2. 小幅波动识别：在0.2%-1%的价格变化中，累积效应比单次变化更重要
3. 价格位置优先：当价格从低位（<40%）上涨时，即使幅度较小也应给予更多关注
4. 连续变化检测：连续多个周期的同向小幅变化（如+0.1%、+0.2%、+0.15%）应视为上涨趋势
5. 结合技术指标综合判断，RSI中性时更应关注价格行为本身
6. 在低波动环境下，降低操作阈值，0.5%的累积涨幅即可考虑买入信号
...
【🎯 特殊信号识别】
- 低位反弹信号：价格位置<30% + 连续上涨 + RSI脱离超卖
- 突破确认信号：价格突破当日区间中轨 + 趋势强度>0.25
- 累积效应信号：5个周期内累计涨幅>0.8%且无明显回调
