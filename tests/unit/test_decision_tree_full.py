...
class TestDecisionEngineDecisionTree:
    ...
    def test_BUY_normal_success(self):
        ...
        result = engine.make_decision(
            "BUY",
            make_selected("BUY", 0.8),
            make_market_data(risk_reward_ratio=2.0, market_structure="bullish"),
        )
        assert result["action"] == "open"

    def test_BUY_final_confidence_below_threshold_blocks(self):
        """BUY + 最终集成置信度低于阈值 → skip"""
        engine = self._make_engine()
        result = engine.make_decision(
            "BUY",
            make_selected("BUY", 0.8),
    ...
    def test_BUY_high_risk_low_confidence_blocks(self):
        """BUY + BTC高位风险且最终置信度很低 → skip"""
        engine = self._make_engine()
        result = engine.make_decision(
            "BUY",
            make_selected("BUY", 0.8),
    ...
    def test_SHORT_normal_success(self):
        ...
        engine = self._make_engine()
        result = engine.make_decision(
            "SHORT", make_selected("SHORT"), make_market_data()
        )
    ...
    def test_SHORT_with_position_closes(self):
        ...
        engine = self._make_engine()
        result = engine.make_decision(
            "SHORT", make_selected("SHORT"), make_market_data(has_position=True)
        )
    ...
    def test_SELL_no_position_skips(self):
        ...
        engine = self._make_engine()
        result = engine.make_decision(
            "SELL", make_selected("HOLD"), make_market_data(has_position=False)
        )
        assert result["action"] == "skip"
    ...
    def test_SELL_with_position_closes(self):
        ...
        engine = self._make_engine()
        result = engine.make_decision(
            "SELL", make_selected("HOLD"), make_market_data(has_position=True)
        )
        assert result["action"] == "close"
    ...
    def test_SELL_strategy_resonance(self):
        ...
        engine = self._make_engine()
        result = engine.make_decision(
            "SELL", make_selected("SELL"), make_market_data(has_position=True)
        )
        assert result["action"] == "close"
    ...
    def test_HOLD_both_hold_skips(self):
        """HOLD + 策略HOLD → skip"""
        engine = self._make_engine()
        result = engine.make_decision("HOLD", make_selected("HOLD"), make_market_data())
        assert result["action"] == "skip"
    ...
    def test_HOLD_market_structure_short(self):
        """HOLD + HOLD策略 + 短结构+强确认 → sell (市场结构做空)"""
        engine = self._make_engine()
        result = engine.make_decision(
            "HOLD",
            make_selected("HOLD"),
    ...
    def test_HOLD_market_structure_short_weak_trend_blocks(self):
        """HOLD + 短结构但趋势弱 → skip，避免震荡里过度做空。"""
        engine = self._make_engine()
        result = engine.make_decision(
            "HOLD",
            make_selected("HOLD"),
    ...
    def test_HOLD_market_structure_short_rr_not_strong_blocks(self):
        """HOLD + 短结构但R/R未达强覆盖阈值 → skip。"""
        engine = self._make_engine()
        result = engine.make_decision(
            "HOLD",
            make_selected("HOLD"),
    ...
    def test_HOLD_market_structure_short_low_final_confidence_blocks(self):
        """HOLD + 短结构但最终置信度不足 → skip。"""
        engine = self._make_engine()
        result = engine.make_decision(
            "HOLD",
            make_selected("HOLD"),
    ...
    def test_HOLD_strategy_sell(self):
        ...
        engine = self._make_engine()
        result = engine.make_decision(
            "HOLD", make_selected("SELL", 0.8), make_market_data()
        )
        assert result["action"] == "skip"
    ...
    def test_unknown_signal_uses_strategy(self):
        ...
        engine = self._make_engine()
        result = engine.make_decision(
            "UNKNOWN", make_selected("BUY", 0.7), make_market_data()
        )
        assert result["action"] == "open"
    ...
    def test_unknown_signal_strategy_hold(self):
        ...
        engine = self._make_engine()
        result = engine.make_decision(
            "UNKNOWN", make_selected("HOLD"), make_market_data()
        )
        assert result["action"] == "skip"
    ...
    def test_safe_mode_reduced_open(self):
        ...
        result = engine.make_decision(
            "BUY",
            make_selected("BUY", 0.7, strategy_type="safe_mode"),
            make_market_data(
